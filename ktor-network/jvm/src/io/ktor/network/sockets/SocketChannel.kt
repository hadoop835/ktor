...
internal fun WritableByteChannel.write(buffer: Buffer): Long {
    ...
    return count
//    if (buffer.exhausted()) return 0
//
//    var rc = 0
//    var done = false
//    while (!done && !buffer.exhausted()) {
//        UnsafeBufferAccessors.readFromHead(buffer) { buffer, start, end ->
//            val count = write(ByteBuffer.wrap(buffer, start, end - start))
//            if (count == 0) done = true
//            rc += count
//            return@readFromHead count
//        }
//    }
//
//    return rc
